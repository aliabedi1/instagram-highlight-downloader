...
Keepsake is a local Instagram highlight downloader for Windows. Connect an
Instagram account, paste a profile link, browse its highlight stories in
Instagram order, and download one story, one highlight, or the complete
collection as a ZIP.
...
`start-local.ps1` starts the local Python backend at
